...
```
[ACTION] 读取 ZDS 设计规范：
1. .harnessdesign/knowledge/Design.md（全局视觉规则：色彩、间距、字体、圆角、阴影、布局）
2. .harnessdesign/knowledge/zds-index.md（组件索引 L0）
3. 根据 03-design-contract.md 中引用的 [ZDS:xxx] 组件，按需读取：
   .harnessdesign/knowledge/zds/components/<component-name>.md（组件详细规范 L2）
```
...
TODO（Phase 6 激活）：
  ...
  调用 .harnessdesign/scripts/dom_assembler.py 执行确定性 DOM 操作。
```
